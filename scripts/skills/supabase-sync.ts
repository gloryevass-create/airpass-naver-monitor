...
async function main() {
  ...
  if (!filePath) {
    console.error(
      "사용법: tsx scripts/skills/supabase-sync.ts <data/processed/{ads,blog,news,budget,prespec,youtube,events,youthfacilities,disabilityorgs,disabilitysports,disabilitywelfare,specialschools,publicinstitutions,seniorwelfare}_YYYY-MM-DD.json>"
    );
    process.exit(1);
  }
