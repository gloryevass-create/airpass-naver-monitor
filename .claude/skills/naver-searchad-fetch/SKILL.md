...
description: naver-keyword-sync가 확정한 키워드들의 월간검색수·경쟁정도를 네이버 검색광고 API로 조회한다(A2). ad-monitor 두 번째 단계.
---
...
5. "우리 순위"(`our_rank`)는 이 API가 아니라 `naver-rank-tracker`(A3, 검색광고 공식 통계
   `/stats`의 `avgRnk`)에서 채워진다 — 이 스킬은 검색량/경쟁정도만 담당한다.

## 주의사항

- 계정에 등록된 키워드 수가 많으면(실 계정 기준 900개 이상) 배치 호출 사이에 짧은 지연을 두는 것을 고려한다.
